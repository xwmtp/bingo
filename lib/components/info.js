 ...
 * @returns
 */
const template = ({ newSeed, version, lang, mode }) => `
  <div class="info">
  ...
  version ? `&version=${version}` : ""
    ...
    <a class="link-button" href="/bingo/goal-list.html?version=${version}&mode=${mode}">${localize("Goal list")}</a>
    </div>
 ...
 * @returns
 */
export const renderInfo = ({ version, mode }) => {
  const search = new URLSearchParams(location.search);
  const newSeed = Math.ceil(999999 * Math.random());
  return fromHtml(template({ newSeed, lang: search.get("lang"), version, mode }));
};
